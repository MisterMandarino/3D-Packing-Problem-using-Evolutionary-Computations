...
with open(os.path.join('data',FILE)) as f:
    for index, line in enumerate(f.readlines()):
        ...
        if len(separated_values) == 3:
            items.append((separated_values[0], separated_values[1], separated_values[2]))

## Integrity Check
for item in items:
    assert (item[0] <= W or item[0] <= H or item[0] <= D) and \
           (item[1] <= W or item[1] <= H or item[1] <= D) and \
           (item[2] <= W or item[2] <= H or item[2] <= D), 'The items are too big to fit into the container'
assert sum([item[0]*item[1]*item[2] for item in items]) <= (W * H * D), 'The items cannot fit into the container'
...
if PLOT:
    if MODE:
        plot_configuration_3d_interactive_with_rotation(population[0], items, W, H, D)
    else:
        plot_configuration_3d_with_rotation(population[0], items, W, H, D, multi_color=True)
